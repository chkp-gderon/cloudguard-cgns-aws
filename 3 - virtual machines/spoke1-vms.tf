...
resource "aws_instance" "spoke1_windows_server" {
  ...
  ebs_block_device {
    ...
    network_interface {
      network_interface_id = aws_network_interface.spoke1_win_eni.id
      device_index = 0
    }
    tags = merge(
    { Name = join("-", ["spoke1", "windows", "server", var.region]) },
    var.tags
  )
}
# Create Elastic IP for the EC2 instance
# resource "aws_eip" "spoke1_windows_eip" {
#   domain = "vpc"
#   tags = merge(
...
#   )
# }
resource "aws_network_interface" "spoke1_win_eni" {
  subnet_id                   = data.aws_subnet.spoke1_pub_1a_id.id
  private_ips = [cidrhost(data.aws_subnet.spoke1_pub_1a_id.cidr_block, 10)]
  private_ips_count = 0
  source_dest_check           = false
  security_groups      = [aws_security_group.spoke1_sg_all.id]
}
...
#   network_interface_id   = aws_network_interface.spoke1_win_eni.id
#   allocation_id = aws_eip.spoke1_windows_eip.id
# }

resource "aws_security_group" "spoke1_sg_all" {
  name       = join("-", ["spoke1", var.region])
  description = "Testing SG"
  vpc_id      = data.aws_vpc.spoke1.id 
...
resource "aws_instance" "spoke1_ubuntu" {
  ...
  ebs_block_device {
    ...
    network_interface {
      network_interface_id = aws_network_interface.spoke1_ubuntu_eni.id
      device_index = 0
    }
  tags = merge(
    { Name = join(".", ["spoke1",  "ubuntu", var.region]) },
    var.tags
  )
  # lifecycle {
  #   ignore_changes =  [tags ]
...
# resource "aws_eip" "spoke1_ubuntu_eip" {

#     domain = "vpc"
#   tags = merge(
#     { Name = join(".", ["spoke1", "EIP", "ubuntu", var.region]) },
#     var.tags
#   )
# }
# # Associate Elastic IP to Server
# resource "aws_eip_association" "eipa_spoke1_ubuntu" {
#   network_interface_id =  aws_network_interface.spoke1_ubuntu_eni.id
#   allocation_id = aws_eip.spoke1_ubuntu_eip.id
# }

resource "aws_network_interface" "spoke1_ubuntu_eni" {
  subnet_id                   = data.aws_subnet.spoke1_pub_1a_id.id
  private_ips = [cidrhost(data.aws_subnet.spoke1_pub_1a_id.cidr_block, 12)]
  private_ips_count = 0
  source_dest_check           = false
  security_groups      = [aws_security_group.spoke1_sg_all.id]
}
